...
def parse_and_execute(tokens)
  ...
  when 'create_parking_lot'
    raise INVALID_NUM_SLOTS_ERROR unless (tokens[1].to_i > 0)
    create_parking_lot(size: tokens[1].to_i)

  when 'park'
    raise EMPTY_PLATE_NUMBER_COLOR_ERROR unless tokens.size >= 3
    park(plate: tokens[1], color: tokens[2])

  when 'leave'
    raise INVALID_SLOT_NUMBER_ERROR unless (tokens[1].to_i > 0)
  ...
  when 'registration_numbers_for_cars_with_colour'
    raise EMPTY_COLOR_ERROR unless tokens.size >= 2
